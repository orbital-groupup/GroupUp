...
Meteor.methods({
	...
	autoAllocateTasks: function(tasks, groupId){
		...
		_.each(tasks, function(task){
			...
			Meteor.call('createUserTaskNotification', task, lowest.name, function(err, res){
				if (err){
					console.log(err.reason);
				}
			});

			Meteor.call('insertEmail', email, task.reminder, task._id, function(err, res){
			if (err)
				console.log(err.reason);
			else
				return res;
	...
	taskRemove: function(taskId){
		check(taskId, String);
		check(Meteor.userId(), String);
		Tasks.remove({_id: taskId, author: Meteor.user().username});
	}
