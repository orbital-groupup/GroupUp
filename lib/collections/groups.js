...
Meteor.methods({
	...
	joinGroup: function(groupAttributes) {
		...
		var groupId = Groups.update({_id: groupAttributes.groupId}, {$push: {members: {name: Meteor.user().username, expectedPoints: 0, actualPoints: 0}} });
	...
	inviteToGroup: function(groupAttributes){
		...
		var groupId = Groups.update({_id: groupAttributes.groupId}, {$push: {members: {name: groupAttributes.username, expectedPoints: 0, actualPoints: 0}}});
	...
	updateActualPoints: function(groupAttributes){
		...
		Groups.update({_id: groupAttributes.groupId, "members.name": Meteor.user().username }, {$inc: {"members.$.actualPoints": groupAttributes.points} });
	},

	updateExpectedPoints: function(groupAttributes){
		check(Meteor.userId(), String);
		check(groupAttributes, {
			groupId: String,
			points: Number
		...
		Groups.update({_id: groupAttributes.groupId, "members.name": Meteor.user().username}, {$inc: {"members.$.expectedPoints": groupAttributes.points}});
	}
});